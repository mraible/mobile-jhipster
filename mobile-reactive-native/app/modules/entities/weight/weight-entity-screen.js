...
import { weightEntityDetailScreen, weightEntityEditScreen } from '../../../navigation/layouts'
import SearchBar from '../../../shared/components/search-bar/search-bar'
import WeightActions from './weight.reducer'
import styles from './weight-entity-screen-style'
...
class WeightEntityScreen extends React.PureComponent {
  ...
  navigationButtonPressed({ buttonId }) {
    weightEntityEditScreen({ entityId: null })
  }
  /* ***********************************************************
  * STEP 2
  ...
  *************************************************************/
  renderRow({ item }) {
    return (
      <TouchableOpacity onPress={weightEntityDetailScreen.bind(this, { entityId: item.id })}>
  ...
  renderEmpty = () => <AlertMessage title="No Weights Found" show={!this.props.fetching} />
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.weights) {
      return {
        done: nextProps.weights.length < prevState.size,
        dataObjects: [...prevState.dataObjects, ...nextProps.weights],
      }
    }
    return null
  }
