...
export class PreferencesPage {
    ...
    async loadAll(refresher?) {
        this.preferencesService.query().pipe(
    ...
    async delete(preferences) {
        this.preferencesService.delete(preferences.id).subscribe(async () => {
            const toast = await this.toastCtrl.create(
                {message: 'Preferences deleted successfully.', duration: 3000, position: 'middle'});
            toast.present();
            this.loadAll();
