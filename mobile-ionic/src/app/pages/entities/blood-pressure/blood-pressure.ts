...
export class BloodPressurePage {
    ...
    async loadAll(refresher?) {
        this.bloodPressureService.query().pipe(
    ...
    async delete(bloodPressure) {
        this.bloodPressureService.delete(bloodPressure.id).subscribe(async () => {
            const toast = await this.toastCtrl.create(
                {message: 'BloodPressure deleted successfully.', duration: 3000, position: 'middle'});
            toast.present();
            this.loadAll();
