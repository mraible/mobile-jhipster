...
describe('Weight e2e test', () => {
  ...
  it('should load Weights', async () => {
    ...
    await element.all(by.css('ion-item'))
      .filter(async el => (await el.element(by.css('h2')).getText()) === 'Weight').first().click();

    weightComponentsPage = new WeightComponentsPage();
    await browser.wait(ec.visibilityOf(weightComponentsPage.title), 5000);
    expect(await weightComponentsPage.getTitle())
  ...
  it('should create Weight', async () => {
    ...
    expect(await weightUpdatePage.getPageTitle())
    ...
    expect(await weightComponentsPage.getTitle())
      .toEqual(COMPONENT_TITLE);
    expect(await weightComponentsPage.getEntitiesNumber())
  ...
  it('should view the last Weight', async () => {
    ...
    expect(await weightDetailPage.getPageTitle())
      .toEqual(SUBCOMPONENT_TITLE);
